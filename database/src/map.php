    ...
    while($row=$result->fetch_assoc()){
        ...
        foreach($evt_id as $id){
            if($note[$ii] != null){
                ...
                <tr><td>Seismic station?</td><td>$inst[$ii]</td></tr>
                <tr><td>Field performance</td><td>$desc_fp[$ii]</td></tr>";
...
function LOCA_F($result){
    ...
    while($row=$result->fetch_assoc()){
        ...
        <table width='400px'>
            ...
            <tr><td>Data</td><td>assoc. files</td></tr>
            <tr><td valign='top'><a href='$output_file'>$loca_id.csv</a> <input type='button' value='plot' onclick='bhplot()'></td>
        ";
        if($assoc_file != null){
            $desc = $desc."<td valign='top'>$assoc_file</td>";
        }
...
function FLDO_F($result){
    ...
    while($row=$result->fetch_assoc()){
        ...
        "<table width='400px'>
            ...
            <tr><td>Measured disp. (x / y / z) (m)</td><td>$xyz</td></tr>
...
if(isset($_POST['btn-filter']))
{
    ...
    $prep_stmt = "
        SELECT PROJ.*
        FROM PROJ $inner
        WHERE PROJ.status='COMPLETE' $mdspsql $ldfmsql $sttlsql $sndbsql $pedfsql $s_typesql $evtsql $Msql[0] $Msql[1] $PGAsql[0] $PGAsql[1] $PGVsql[0] $PGVsql[1] 
    ...
    foreach($LOCA as $loca){
        ...
        $prep_stmt = "
            ...
            FROM $group $inner
            WHERE PROJ.status='COMPLETE' AND $group.loca_type='$loca' $mdspsql $ldfmsql $sttlsql $sndbsql $pedfsql $s_typesql $evtsql $Msql[0] $Msql[1] $PGAsql[0] $PGAsql[1] $PGVsql[0] $PGVsql[1] 
            GROUP BY $group.id
            ";
        $result = $mysqli->query($prep_stmt);
        ...
        $prep_stmt = "
            SELECT $group.*, PROJ.site_name 
            FROM $group $inner
            WHERE PROJ.status='COMPLETE' AND LOCA.loca_type='$loca' $mdspsql $ldfmsql $sttlsql $sndbsql $pedfsql $s_typesql $evtsql $Msql[0] $Msql[1] $PGAsql[0] $PGAsql[1] $PGVsql[0] $PGVsql[1] 
            GROUP BY $group.id";
        $result = $mysqli->query($prep_stmt);
...
<body>
    ...
    <div id="map_query">
        ...
        <tr>
            <td>
                <input type='radio' name='s_type' value='phto' <?if($s_type=='phto') echo('checked')?> > Satel. Image
            </td>
            <td>
    ...
    <script>
        ...
        // Map image from ESRI
        var map = L.map('map_database').setView([0,45], 2);
